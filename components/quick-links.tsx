      ...
      <div className="container">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 md:gap-6">
          {links.map((link, index) => {
            ...
            return (
              <FadeIn key={link.title} delay={index * 80} direction="up">
                <Link href={link.href} className="group block">
                  <Card className="h-full transition-all hover:shadow-md hover:-translate-y-0.5 border-border/60">
                    <CardContent className="p-3 sm:p-5 md:p-8 flex flex-col items-center text-center gap-2 md:gap-3">
                      <div className="h-10 w-10 sm:h-14 sm:w-14 md:h-16 md:w-16 rounded-full bg-primary/10 flex items-center justify-center group-hover:bg-primary/20 transition-colors">
                      ...
                      <div>
                        ...
                        <p className="text-xs sm:text-sm md:text-base text-muted-foreground leading-relaxed hidden md:block mt-1">{link.description}</p>
                      </div>
                    </CardContent>
                  </Card>
                </Link>
